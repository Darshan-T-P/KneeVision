...
def main():
    ...
    parser.add_argument("--lr", type=float, default=CLINICAL_LEARNING_RATE)
    ...
    if args.synthetic:
        ...
        splits = load_reports_csv(args.data)
    else:
    ...
    val_texts = splits.get("val", (None, None))[0]
    val_labels = splits.get("val", (None, None))[1]
    test_texts = splits.get("test", (None, None))[0]
    test_labels = splits.get("test", (None, None))[1]
    ...
    train_ds = ClinicalTextDataset(train_texts, train_labels, max_length=CLINICAL_MAX_LENGTH)
    train_loader = DataLoader(train_ds, batch_size=args.batch_size, shuffle=True, num_workers=0)
    val_loader = test_loader = None
    if val_texts:
        val_loader = DataLoader(ClinicalTextDataset(val_texts, val_labels, max_length=CLINICAL_MAX_LENGTH),
                                batch_size=args.batch_size, num_workers=0)
    if test_texts:
        test_loader = DataLoader(ClinicalTextDataset(test_texts, test_labels, max_length=CLINICAL_MAX_LENGTH),
                                 batch_size=args.batch_size, num_workers=0)
    ...
    if test_loader is not None:
        test_loss, test_acc, test_kappa, preds, labels = validate(model, test_loader, criterion, device)
        logger.info("=" * 60)
